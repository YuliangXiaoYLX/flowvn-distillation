...
class VnMriReconCellLowMem(nn.Module):
    def __init__(self, block_id, **kwargs):
        ...
        conv_kernel_xyz /= (options["kernel_size"] ** 2 * options["features_in"]) ** 0.5
        ...
        conv_kernel_xyt /= (options["kernel_size"] ** 2 * options["features_in"]) ** 0.5
        ...
        conv_kernel_yzt /= (options["kernel_size"] ** 2 * options["features_in"]) ** 0.5
        ...
        conv_kernel_xzt /= (options["kernel_size"] ** 2 * options["features_in"]) ** 0.5
    ...
    def forward(self, u_t_1, f, c, usrate, S_prev=None):
        ...
        if self.use_usrate_modulation:
            usrate_input = usrate.unsqueeze(0) if usrate.dim() == 0 else usrate
            lamb_ru = self.lamb_ru_modulation(usrate_input).view(-1, 1, 1, 1, 1, 1, 1)
            lamb_du = self.lamb_du_modulation(usrate_input).view(-1, 1, 1, 1, 1, 1, 1)
        else:
            lamb_ru = self.lamb_ru
            lamb_du = self.lamb_du

        G = torch.zeros_like(u_t_1)

        for t in range(T):
            x = u_t_1[:, :, t]
            x = x.permute(0, 5, 1, 2, 3, 4).contiguous()
            x = x.view(N * 2, V, D, H, W)
            x = F.conv3d(x, self.conv_kernel_xyz, padding=(self.pad_d, self.pad, self.pad))
            x = self.activation1(x)
            x = F.conv_transpose3d(x, self.conv_kernel_xyz, padding=(self.pad_d, self.pad, self.pad))
            x = x.view(N, 2, V, D, H, W).permute(0, 2, 3, 4, 5, 1)
            G[:, :, t].add_(x, alpha=1.0)
            del x

        for w in range(W):
            x = u_t_1[:, :, :, :, :, w]
            x = x.permute(0, 5, 1, 3, 4, 2).contiguous()
            x = x.view(N * 2, V, D, H, T)
            x = F.conv3d(x, self.conv_kernel_xyt, padding=(self.pad_d, self.pad, self.pad))
            x = self.activation2(x)
            x = F.conv_transpose3d(x, self.conv_kernel_xyt, padding=(self.pad_d, self.pad, self.pad))
            x = x.view(N, 2, V, D, H, T).permute(0, 2, 5, 3, 4, 1)
            G[:, :, :, :, :, w].add_(x, alpha=1.0)
            del x

        for d in range(D):
            x = u_t_1[:, :, :, d]
            x = x.permute(0, 5, 1, 3, 4, 2).contiguous()
            x = x.view(N * 2, V, H, W, T)
            x = F.conv3d(x, self.conv_kernel_yzt, padding=(self.pad, self.pad, self.pad))
            x = self.activation3(x)
            x = F.conv_transpose3d(x, self.conv_kernel_yzt, padding=(self.pad, self.pad, self.pad))
            x = x.view(N, 2, V, H, W, T).permute(0, 2, 5, 3, 4, 1)
            G[:, :, :, d].add_(x, alpha=1.0)
            del x

        for h in range(H):
            x = u_t_1[:, :, :, :, h]
            x = x.permute(0, 5, 1, 3, 4, 2).contiguous()
            x = x.view(N * 2, V, D, W, T)
            x = F.conv3d(x, self.conv_kernel_xzt, padding=(self.pad_d, self.pad, self.pad))
            x = self.activation4(x)
            x = F.conv_transpose3d(x, self.conv_kernel_xzt, padding=(self.pad_d, self.pad, self.pad))
            x = x.view(N, 2, V, D, W, T).permute(0, 2, 5, 3, 4, 1)
            G[:, :, :, :, h].add_(x, alpha=1.0)
            del x

        G.div_(self.options["features_out"])
        G.mul_(lamb_ru)
        ...
        rr = residual.real.view(N, V, C * T, D, H * W)
        ri = residual.imag.view(N, V, C * T, D, H * W)
        rr = self.activation5(rr).view(N, V, C, T, D, H, W)
        ri = self.activation5(ri).view(N, V, C, T, D, H, W)
        residual = rr + 1j * ri

        Du = torch.view_as_real(mri_adjoint_op(residual, c))
        G.add_(Du, alpha=1.0)
        G.mul_(lamb_du / (lamb_ru + 1e-12))

        S = G if (S_prev is None or not self.sgd_momentum) else G + self.alpha * S_prev
        u_next = u_t_1 - S
        return u_next, S

class FlowVNLowMem(nn.Module):
    def __init__(self, **kwargs):
        super(FlowVNLowMem, self).__init__()
        options = kwargs
        self.options = options
        ...
        for i in range(self.nc):
            cells.append(VnMriReconCellLowMem(block_id=i, **options))
