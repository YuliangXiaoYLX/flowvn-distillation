...
def _flowvn_checkerboard_sign(reference, output_phase=False):
    """Broadcastable +/-1 sign map for centered FFT checkerboard modulation."""
    h = int(reference.shape[-2])
    w = int(reference.shape[-1])
    if h % 2 != 0 or w % 2 != 0:
        raise ValueError(
            f"Checkerboard centered FFT requires even spatial dimensions, got {(h, w)}"
        )
    dtype = reference.real.dtype if torch.is_complex(reference) else reference.dtype
    rows = torch.arange(h, device=reference.device).view(h, 1)
    cols = torch.arange(w, device=reference.device).view(1, w)
    offset = (h // 2 + w // 2) if output_phase else 0
    parity = (rows + cols + offset).remainder(2)
    sign = torch.where(parity == 0, 1.0, -1.0).to(dtype=dtype)
    return sign.view(*([1] * (reference.ndim - 2)), h, w)


def make_flowvn_dc_checkerboard_cache(f, coil_sens):
    """Precompute static factors for the checkerboard centered-FFT FlowVN DC path."""
    sign_in_c = _flowvn_checkerboard_sign(coil_sens, output_phase=False)
    sign_out_c = _flowvn_checkerboard_sign(coil_sens, output_phase=True)
    sign_out_f = _flowvn_checkerboard_sign(f, output_phase=True)
    sign_in_f = _flowvn_checkerboard_sign(f, output_phase=False)
    sampling_mask = torch.abs(f[:, :, 0, :, 0, :, :]) != 0
    return {
        "c_forward": coil_sens * sign_in_c,
        "c_adjoint": torch.conj(coil_sens) * sign_out_c,
        "signed_mask": sampling_mask.unsqueeze(2).unsqueeze(4) * sign_out_f,
        "sign_in_f": sign_in_f,
    }


def mri_forward_op_checkerboard(u, dc_cache):
    """Forward MRI op using checkerboard centered FFT instead of fftshift rolls."""
    coil_imgs = u.unsqueeze(2) * dc_cache["c_forward"].unsqueeze(1).unsqueeze(3)
    Fu = torch.fft.fft2(coil_imgs, dim=(-2, -1), norm="ortho")
    return dc_cache["signed_mask"] * Fu


def mri_adjoint_op_checkerboard(f, dc_cache):
    """Adjoint MRI op using checkerboard centered IFFT instead of fftshift rolls."""
    Finv = torch.fft.ifft2(f * dc_cache["sign_in_f"], dim=(-2, -1), norm="ortho")
    coil_sens = dc_cache["c_adjoint"].unsqueeze(1).unsqueeze(3)
    return torch.sum(Finv * coil_sens, 2)
